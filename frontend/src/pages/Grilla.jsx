...
export default function Grilla({ user, onNav, onLogout }) {
  ...
  const handleDuplicarSemana = async () => {
    const origen = prompt(
      "Fecha de inicio de la semana ORIGEN (YYYY-MM-DD):",
      fecha
    );
    if (!origen) return;
    const destino = prompt(
      "Fecha de inicio de la semana DESTINO (YYYY-MM-DD):",
      ""
    );
    if (!destino) return;
    ...
    try {
      const body = {
        fecha_inicio_origen: origen,
        fecha_inicio_destino: destino,
      ...
      alert(
        `Semana duplicada: ${res.horarios_duplicados} horarios, ` +
        `${res.asignaciones_duplicadas} asignaciones.` +
        (res.advertencias?.length ? `\nAdvertencias: ${res.advertencias.length}` : "")
      );
      setFecha(destino);
    } catch (e) {
      alert(e.message || "No se pudo duplicar la semana.");
    } finally {
