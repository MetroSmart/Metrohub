...
_CARBON    = colors.HexColor("#1E1E2E")   # encabezados y secciones
_GRIS_OSC  = colors.HexColor("#4A4A5A")   # texto secundario / pie
_GRIS_MED  = colors.HexColor("#9A9AAA")   # bordes suaves
_GRIS_CLR  = colors.HexColor("#F4F4F6")   # filas alternas
_ALERTA    = colors.HexColor("#F0EDED")   # fila urgente — casi blanco cálido
_BLANCO    = colors.white
_TEXTO     = colors.HexColor("#1A1A2A")


def _pie_pagina(canvas, doc):
    canvas.saveState()
    canvas.setFillColor(_CARBON)
    canvas.rect(0, 0, A4[0], 1.1 * cm, fill=1, stroke=0)
    canvas.setFont("Helvetica", 7)
    canvas.setFillColor(_GRIS_OSC)
    canvas.drawString(
        2 * cm, 0.38 * cm,
...
def _tbl_style(extra_cmds=None) -> list:
    ...
    return base + (extra_cmds or [])


def _make_tbl(data, widths, extra_cmds=None):
    tbl = Table(data, colWidths=widths)
    tbl.setStyle(TableStyle(_tbl_style(extra_cmds)))
    return tbl


def _seccion_titulo(texto: str, styles, alerta=False):
    color = colors.HexColor("#3A1A1A") if alerta else _CARBON
    tbl = Table(
        [[Paragraph(
            texto,
            ParagraphStyle(
...
class PdfExporter(ExportadorReporte):
    def exportar(self, datos: dict[str, Any]) -> tuple[bytes, str, str]:
        ...
        note_s = ParagraphStyle(
            ...
            fontSize=8, textColor=_GRIS_OSC, spaceAfter=4, leftIndent=4,
        )
        ...
        header_data = [[
            ...
            Paragraph(
                f'<font color="#9A9AAA">Fecha:&nbsp;</font>'
                f'<font color="white"><b>{datos.get("fecha", "")}</b></font>',
                ParagraphStyle(
                    "hd", parent=styles["Normal"],
                    fontSize=9, textColor=_GRIS_OSC,
        ...
        if kpis:
            ...
            for i in range(mid):
                ...
                rows.append([
                    ...
                    Paragraph(f"<b>{lv}</b>", val_s),
                    Paragraph(_KPI_LABELS.get(rk, rk.replace("_", " ").capitalize()) if rk else "", cell_s),
                    Paragraph(f"<b>{rv}</b>" if rv != "" else "", val_s),
                ])
            kpi_tbl = Table(rows, colWidths=[6 * cm, 1.8 * cm, 6 * cm, 1.8 * cm])
            kpi_tbl.setStyle(TableStyle([
                ("ROWBACKGROUNDS", (0, 0), (-1, -1), [_BLANCO, _GRIS_CLR]),
                ("GRID",          (0, 0), (-1, -1), 0.3,  _GRIS_MED),
                ("LINEAFTER",     (1, 0), (1, -1),  0.5,  _GRIS_MED),
                ("VALIGN",        (0, 0), (-1, -1), "MIDDLE"),
                ("TOPPADDING",    (0, 0), (-1, -1), 6),
                ("BOTTOMPADDING", (0, 0), (-1, -1), 6),
                ("LEFTPADDING",   (0, 0), (-1, -1), 10),
        ...
        if alertas:
            ...
            for i, a in enumerate(alertas, start=1):
                ...
                if d_lic <= 7 or d_cer <= 7:
                    extra.append(("BACKGROUND", (0, i), (-1, i), _ALERTA))
                    extra.append(("FONTNAME",   (0, i), (-1, i), "Helvetica-Bold"))
            story.append(_make_tbl(
                tdata, [4.5 * cm, 2.8 * cm, 1.5 * cm, 2.8 * cm, 1.5 * cm],
                extra_cmds=extra,
            ))
        else:
        ...
        if progs:
            ...
            story.append(_make_tbl(tdata, [6.5 * cm, 2.7 * cm, 2.5 * cm, 2.5 * cm]))
        else:
